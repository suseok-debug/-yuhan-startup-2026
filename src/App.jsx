...
const STAGE_CONFIG = {
  1: {
    ...
    items: [
      { id: "meeting", name: "학생 지도비", maxAmt: 200000, hint: "회의 당 학생 1인 2만원 이내, 지도교수 3만원 이내" },
      { id: "expert", name: "전문가 활용비", maxAmt: 200000, hint: "외부 전문가 자문비 (실무경력 3년 이상, 1인 20만원, 1회)" },
    ],
    note: "1단계는 운영지원비(학생 지도비, 전문가 활용비)만 집행 가능합니다."
  },
  2: {
...
const STATUS_LABELS = {
  ...
  pending_review: { label: "교수 검토 대기", color: "bg-amber-100 text-amber-700" },
  revision: { label: "수정 요청", color: "bg-red-100 text-red-600" },
  submitted: { label: "접수 완료", color: "bg-emerald-100 text-emerald-700" },
};
...
<p class="sub">유한대학교 일자리지원처 창업지원센터</p>
<table>
...
function ApplicantView({ onBack }) {
  ...
  const submitForReview=async()=>{
    ...
    const updated={...data,status:"pending_review",updatedAt:now()};
    await store.set(`app:${appId}`,updated);
    const idx=(await store.get("apps-index"))||[];
    await store.set("apps-index",idx.map(x=>x.id===appId?{...x,status:"pending_review",clubName:data.info?.clubName}:x));
    setData(updated);setPage("review_sent");
  };
  const reviewUrl=data?`${BASE_URL}?review=${data.id}&token=${data.reviewToken}`:"";
  const mailtoUrl=data?`mailto:${data.info?.profEmail||""}?subject=${encodeURIComponent(`[창업동아리] ${data.info?.clubName||""} 사업계획서 검토 요청`)}&body=${encodeURIComponent(`안녕하세요 ${data.info?.profName||""} 교수님,\n\n${data.info?.clubName||""} 창업동아리 대표학생 ${data.info?.repName||""}입니다.\n사업계획서 검토를 요청드립니다.\n\n아래 링크를 클릭하시면 사업계획서를 확인하고 승인/수정요청을 하실 수 있습니다.\n\n검토 링크: ${reviewUrl}\n\n감사합니다.`)}`:"";
  ...
  if(page==="review_sent") return (<div className="min-h-screen bg-gradient-to-br from-amber-50 to-blue-50 flex items-center justify-center p-4"><div className="max-w-lg w-full text-center">
    <div className="inline-flex items-center justify-center w-20 h-20 bg-amber-500 rounded-full mb-6 shadow-lg"><span className="text-4xl text-white">📨</span></div>
    <h1 className="text-2xl font-extrabold text-gray-900 mb-2">검토 요청 완료!</h1>
    <p className="text-gray-500 mb-6">지도교수님에게 아래 검토 링크를 보내주세요.</p>
    <div className="bg-white rounded-2xl shadow-lg p-5 mb-4 text-left">
      <div className="flex justify-between text-sm py-2 border-b"><span className="text-gray-500">동아리명</span><span className="font-semibold">{data?.info?.clubName}</span></div>
      <div className="flex justify-between text-sm py-2 border-b"><span className="text-gray-500">지도교수</span><span className="font-semibold">{data?.info?.profName} ({data?.info?.profEmail})</span></div>
      <div className="flex justify-between text-sm py-2 border-b"><span className="text-gray-500">상태</span><StatusBadge status="pending_review"/></div>
      <div className="flex justify-between text-sm py-2"><span className="text-gray-500">신청 ID</span><span className="font-mono text-blue-600 font-bold">{appId}</span></div>
    </div>
    <div className="bg-gray-50 rounded-xl p-4 mb-4 text-left"><p className="text-xs text-gray-500 mb-2 font-bold">📎 교수님 검토 링크</p><p className="text-xs text-blue-600 break-all font-mono bg-white p-2 rounded border">{reviewUrl}</p><div className="flex gap-2 mt-3"><CopyBtn text={reviewUrl} label="링크 복사"/><a href={mailtoUrl} className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold transition">📧 이메일 보내기</a></div></div>
    <p className="text-xs text-amber-600 bg-amber-50 p-3 rounded-lg">⚠ 교수님이 승인하면 최종 접수됩니다. 신청 ID를 반드시 메모해주세요.</p>
  </div></div>);

  const isRevision = data?.status === "revision";
  const canSubmit = data?.status === "draft" || data?.status === "revision";
  const StepComponent=[StepInfo,StepProblem,StepCustomer,StepSolution,StepBusiness,StepTeam][step];
  const isLast=step===STEPS.length-1;

  return (<div className="min-h-screen bg-gray-50">
    <div className="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm"><div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between"><div className="flex items-center gap-2"><span className="text-lg">🚀</span><span className="font-bold text-gray-800 text-sm hidden sm:inline">2026 YUHAN 창업동아리</span></div><div className="flex items-center gap-3"><button onClick={()=>openPdf(data)} className="text-xs px-2.5 py-1 bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-600 font-medium">🖨️ PDF</button><StatusBadge status={data?.status}/><span className={`text-xs px-2 py-0.5 rounded-full ${saving?"bg-amber-100 text-amber-700":"bg-emerald-100 text-emerald-700"}`}>{saving?"저장 중...":"✓ 자동저장"}</span><span className="text-xs text-gray-400 font-mono hidden sm:inline">ID: {appId}</span></div></div></div>
    <div className="max-w-4xl mx-auto px-4 py-6">
      {isRevision && data.profReview?.comment && (<div className="bg-red-50 border-2 border-red-200 rounded-xl p-4 mb-6"><div className="flex items-center gap-2 mb-2"><span className="text-lg">↩</span><span className="font-bold text-red-700">지도교수 수정 요청</span></div><p className="text-sm text-red-600">{data.profReview.comment}</p><p className="text-xs text-red-400 mt-1">{new Date(data.profReview.reviewedAt).toLocaleString("ko-KR")}</p></div>)}
      ...
      <div className="flex justify-between items-center mb-12">
        ...
        {isLast && canSubmit ? <button onClick={submitForReview} className="px-8 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-sm font-bold transition shadow-lg">📨 지도교수 검토 요청</button> : isLast ? <div className="text-sm text-gray-400">{data?.status==="pending_review"?"교수 검토 대기 중":"접수 완료"}</div> : <button onClick={()=>goStep(Math.min(STEPS.length-1,step+1))} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold transition shadow-lg shadow-blue-200">다음 →</button>}
      </div>
    </div>{toast&&<Toast message={toast} onClose={()=>setToast("")}/>}</div>);
}

/* ═══════════════════════════════════════
   Professor Review View (지도교수 검토)
   ═══════════════════════════════════════ */
function ProfessorView({ appId, token }) {
  ...
  useEffect(() => {
    (async () => {
      const d = await store.get(`app:${appId}`);
      if (d && d.reviewToken === token && d.status === "pending_review") setData(d);
      setLoading(false);
    })();
  }, [appId, token]);
  ...
  const handleAction = async (action) => {
    const updated = {
      ...data,
      status: action === "approve" ? "submitted" : "revision",
      profReview: { comment, reviewedAt: now(), action },
      updatedAt: now()
    };
    if (action === "approve") updated.submittedAt = now();
    await store.set(`app:${appId}`, updated);
    const idx = (await store.get("apps-index")) || [];
    await store.set("apps-index", idx.map(x => x.id === appId ? { ...x, status: updated.status } : x));
    setDone(action);
  };
  ...
  if (!data) return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm">유효하지 않은 링크이거나 이미 검토가 완료된 신청서입니다.</p>
      </div>
    </div>
  );
  ...
  if (done) return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full text-center">
        <div className={`inline-flex items-center justify-center w-20 h-20 rounded-full mb-6 shadow-lg ${done === "approve" ? "bg-emerald-500" : "bg-amber-500"}`}>
          <span className="text-4xl text-white">{done === "approve" ? "✓" : "↩"}</span>
        </div>
        <h1 className="text-2xl font-extrabold text-gray-900 mb-2">{done === "approve" ? "승인 완료!" : "수정 요청 완료!"}</h1>
        <p className="text-gray-500">{done === "approve" ? "사업계획서가 최종 접수되었습니다." : "학생에게 수정 요청이 전달되었습니다."}</p>
      </div>
    </div>
  );
  ...
  const stg = STAGE_CONFIG[parseInt(info.stage) || 1];
  const sections = [
    { t: "1. 창업 동기 및 문제 발견", c: [data.problem?.contextResult, data.problem?.marketContext] },
    { t: "2. 고객 이해 및 시장 탐색", c: [data.customer?.painResult, data.customer?.marketResult] },
    { t: "3. 솔루션 및 아이템 구체화", c: [data.solution?.featureResult, data.solution?.devResult] },
    { t: "4. 비즈니스 모델 및 성장 전략", c: [data.business?.modelResult, data.business?.sustainability] },
    { t: "5. 팀 역량 및 학습 계획", c: [data.team?.roleResult] },
  ];
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-6 mb-6 text-white">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-xl font-extrabold">지도교수 검토</h1>
              <p className="text-blue-200 text-sm">사업계획서를 검토하고 승인 또는 수정 요청을 해주세요.</p>
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
            <div className="bg-white/10 rounded-lg p-2.5"><span className="text-blue-200 text-xs block">동아리</span><span className="font-bold">{info.clubName}</span></div>
            <div className="bg-white/10 rounded-lg p-2.5"><span className="text-blue-200 text-xs block">아이템</span><span className="font-bold">{info.itemName}</span></div>
            <div className="bg-white/10 rounded-lg p-2.5"><span className="text-blue-200 text-xs block">대표학생</span><span className="font-bold">{info.repName}</span></div>
            <div className="bg-white/10 rounded-lg p-2.5"><span className="text-blue-200 text-xs block">단계</span><span className="font-bold">{info.stage}단계 ({stg.name})</span></div>
          </div>
        </div>

        {/* Content sections */}
        {sections.map((sec, i) => (
          <div key={i} className="bg-white rounded-2xl shadow-sm border p-6 mb-4">
            <h3 className="font-bold text-gray-800 mb-3">{sec.t}</h3>
            {sec.c.map((c, j) => c ? <p key={j} className="text-sm text-gray-600 whitespace-pre-wrap mb-3 leading-relaxed">{c}</p> : null)}
            {sec.c.every(c => !c) && <p className="text-sm text-gray-300 italic">내용이 입력되지 않았습니다.</p>}
          </div>
        ))}

        {/* Review action */}
        <div className="bg-white rounded-2xl shadow-sm border p-6 mb-6">
          <h3 className="font-bold text-gray-800 mb-4">검토 의견</h3>
          <textarea value={comment} onChange={e => setComment(e.target.value)}
            placeholder="승인 또는 수정 요청 시 코멘트를 작성해주세요 (수정 요청 시 필수)" rows={4}
            className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm resize-y mb-4" />
          <div className="flex gap-3">
            <button onClick={() => handleAction("approve")}
              className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-sm transition shadow-lg">
              ✓ 승인 (최종 접수)
            </button>
            <button onClick={() => {
              if (!comment.trim()) { alert("수정 요청 시 코멘트를 반드시 작성해주세요."); return; }
              handleAction("reject");
            }}
              className="flex-1 py-3 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-bold text-sm transition shadow-lg">
              ↩ 수정 요청 (반려)
            </button>
          </div>
        </div>
...
function AdminView({ onBack }) {
  ...
  const counts={all:apps.length,submitted:apps.filter(a=>a.status==="submitted").length,pending_review:apps.filter(a=>a.status==="pending_review").length,draft:apps.filter(a=>a.status==="draft"||a.status==="revision").length};
...
export default function App() {
  ...
  // URL 파라미터 확인: ?review=APP_ID&token=TOKEN → 교수 검토 화면
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const reviewId = params.get("review");
    const token = params.get("token");
    if (reviewId && token) setReviewData({ appId: reviewId, token });
  }, []);

  // 교수 검토 링크로 접속한 경우
  if (reviewData) return <ProfessorView appId={reviewData.appId} token={reviewData.token} />;
  ...
  return (<div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div className="relative z-10 max-w-2xl w-full text-center">
      ...
      <p className="text-blue-300/60 text-sm mb-10">유한대학교 일자리지원처 창업지원센터</p>
